...
function new_zmq_to_qp_run_socket()
  ...
  rc = f77_zmq_setsockopt(new_zmq_to_qp_run_socket, ZMQ_SNDTIMEO, 600000, 4)
  if (rc /= 0) then
    stop 'Unable to set send timeout in new_zmq_to_qp_run_socket'
  endif

  rc = f77_zmq_setsockopt(new_zmq_to_qp_run_socket, ZMQ_RCVTIMEO, 600000, 4)
  if (rc /= 0) then
...
IRP_ENDIF
  ...
  rc = f77_zmq_setsockopt(new_zmq_pull_socket,ZMQ_LINGER,600000,4)
  if (rc /= 0) then
...
IRP_ENDIF
  ...
  rc = f77_zmq_setsockopt(new_zmq_push_socket,ZMQ_LINGER,600000,4)
  if (rc /= 0) then
...
!  endif
  ...
  rc = f77_zmq_setsockopt(new_zmq_push_socket, ZMQ_SNDTIMEO, 600000, 4)
  if (rc /= 0) then
