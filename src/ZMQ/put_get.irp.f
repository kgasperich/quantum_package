...
integer function zmq_put_cdvector(zmq_to_qp_run_socket, worker_id, name, x, size_x)
  use f77_zmq
  implicit none
  BEGIN_DOC
! Put the X vector on the qp_run scheduler
  ...
  character*(*)                  :: name
  integer, intent(in)            :: size_x
  complex*16, intent(out)  :: x(size_x)
  integer                        :: rc
  character*(256)                :: msg
  ...
  rc = f77_zmq_send(zmq_to_qp_run_socket,x,size_x*8*2,0)
  if (rc /= size_x*8*2) then
    zmq_put_cdvector = -1
    return
  endif
...
integer function zmq_get_cdvector(zmq_to_qp_run_socket, worker_id, name, x, size_x)
  use f77_zmq
  implicit none
  BEGIN_DOC
! Get psi_coef from the qp_run scheduler
  ...
  if (mpi_master) then
    ...
    rc = f77_zmq_recv(zmq_to_qp_run_socket,x,size_x*8*2,0)
    if (rc /= size_x*8*2) then
      zmq_get_cdvector = -1
      go to 20
    endif
  ...
  IRP_IF MPI
    integer :: ierr
    include 'mpif.h'
    call MPI_BCAST (zmq_get_cdvector, 1, MPI_INTEGER, 0, MPI_COMM_WORLD, ierr)
    if (ierr /= MPI_SUCCESS) then
      print *,  irp_here//': Unable to broadcast zmq_get_cdvector'
      stop -1
    endif
    call MPI_BCAST (x, size_x, MPI_DOUBLE_COMPLEX, 0, MPI_COMM_WORLD, ierr)
    if (ierr /= MPI_SUCCESS) then
      print *,  irp_here//': Unable to broadcast cdvector'
      stop -1
    endif
  IRP_ENDIF
