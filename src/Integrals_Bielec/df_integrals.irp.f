BEGIN_PROVIDER [complex*16, df_integral_array, (ao_tot_num, ao_tot_num, df_tot_num)]
  implicit none
  BEGIN_DOC
  ...
  if (read_df_integral_array) then
    ...
    call read_df_integral_array_file('df_integral_array',df_integral_array,num_kpts)
...
subroutine write_df_integral_array_file(filename, A, n_ao, n_df)
  implicit none
  BEGIN_DOC
! Write the 1-electron integrals stored in A(m,n) into file 'filename'
  ...
  complex*16, intent(in)         :: A(n_ao,n_ao,n_df)
  ...
  allocate( A_re(n_ao,n_ao,n_df), A_im(n_ao,n_ao,n_df))
...
subroutine read_df_integral_array_file(filename, A, n_k)
  implicit none
  BEGIN_DOC
! Read the 1-electron integrals into in A(m,n) from file 'filename'
  ...
  complex*16, intent(out)        :: A(n_k,n_k,n_k)
  double precision, allocatable  :: A_re(:,:,:), A_im(:,:,:)
