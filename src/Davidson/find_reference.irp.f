subroutine find_reference(thresh,n_ref,result)
  ...
  integer :: i,j,istate
  double precision :: E0(1), hii, norm
  complex*16 :: hij
  ...
  print *,  '======================'
  print *,  ''
  print *,  n_ref, ': E0 = ', E0(1) + nuclear_repulsion
  ...
  do i=2,N_det
    ...
    hii = real(hij)
    de = cdabs(conjg(i_H_psi_array(istate))*i_H_psi_array(istate)) / (E0(1) - hii)
    if (dabs(de) > thresh) then
      n_ref += 1
      result(n_ref) = i
      psi_ref_(:,:,n_ref) = psi_det(:,:,i)
      psi_ref_coef_(n_ref,1) = psi_coef(i,istate)
      call u_0_H_u_0(E0(1),psi_ref_coef_,n_ref,psi_ref_,N_int,1,size(psi_ref_coef_,1))
      print *,  n_ref, ': E0 = ', E0(1) + nuclear_repulsion
      call debug_det(psi_ref_(1,1,n_ref),N_int)
    endif
  enddo
