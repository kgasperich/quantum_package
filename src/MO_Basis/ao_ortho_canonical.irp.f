...
BEGIN_PROVIDER [ complex*16, ao_cts_coef_complex, (ao_num,ao_cart_to_sphe_num) ]
 implicit none
 BEGIN_DOC
 ...
 call zlacp2('A',ao_num,ao_cart_to_sphe_num, &
        ...
        ao_cts_coef_complex,size(ao_cts_coef_complex,1))
END_PROVIDER

BEGIN_PROVIDER [ complex*16, ao_cart_to_sphe_overlap, (ao_cart_to_sphe_num,ao_cart_to_sphe_num) ]
 implicit none
 BEGIN_DOC
 ...
 call zgemm('T','N',ao_cart_to_sphe_num,ao_num,ao_num, (1.d0,0.d0), &
   ao_cts_coef_complex,size(ao_cts_coef_complex,1), &
   ao_overlap,size(ao_overlap,1), (0.d0,0.d0), &
   S, size(S,1))

 call zgemm('N','N',ao_cart_to_sphe_num,ao_cart_to_sphe_num,ao_num, (1.d0,0.d0), &
   S, size(S,1), &
   ao_cts_coef_complex,size(ao_cts_coef_complex,1), (0.d0,0.d0), &
...
BEGIN_PROVIDER [ complex*16, ao_ortho_canonical_coef_inv, (ao_num,ao_num)]
 implicit none
 BEGIN_DOC
! ao_ortho_canonical_coef^(-1)
 END_DOC
 call get_inverse_complex(ao_ortho_canonical_coef,size(ao_ortho_canonical_coef,1),&
     ao_num, ao_ortho_canonical_coef_inv, size(ao_ortho_canonical_coef_inv,1))
END_PROVIDER

 BEGIN_PROVIDER [ complex*16, ao_ortho_canonical_coef, (ao_num,ao_num)]
...
! ao_ortho_canonical_coef(i,j) = coefficient of the ith ao on the jth ao_ortho_canonical orbital
  END_DOC
  integer :: i
  ao_ortho_canonical_coef = (0.d0,0.d0)
  do i=1,ao_num
    ao_ortho_canonical_coef(i,i) = (1.d0,0.d0)
  enddo
...
!return
  ...
  if (ao_cartesian) then
    ...
    call ortho_canonical_complex(ao_cart_to_sphe_overlap, size(ao_cart_to_sphe_overlap,1), &
      ao_cart_to_sphe_num, S, size(S,1), ao_ortho_canonical_num)

    call zgemm('N','N', ao_num, ao_ortho_canonical_num, ao_cart_to_sphe_num, (1.d0,0.d0), &
      ao_cts_coef_complex, size(ao_cts_coef_complex,1), &
      S, size(S,1), &
      (0.d0,0.d0), ao_ortho_canonical_coef, size(ao_ortho_canonical_coef,1))

    deallocate(S)
  endif
END_PROVIDER

BEGIN_PROVIDER [complex*16, ao_ortho_canonical_overlap, (ao_ortho_canonical_num,ao_ortho_canonical_num)]
  implicit none
  BEGIN_DOC
...
! Expected to be the Identity
  ...
  do j=1, ao_ortho_canonical_num
    do i=1, ao_ortho_canonical_num
      ao_ortho_canonical_overlap(i,j) = (0.d0,0.d0)
    enddo
  enddo
  do j=1, ao_ortho_canonical_num
    do k=1, ao_num
      c = (0.d0,0.d0)
      do l=1, ao_num
        c +=  conjg(ao_ortho_canonical_coef(l,j)) * ao_overlap(l,k)
      enddo
      do i=1, ao_ortho_canonical_num
