  ...
  do i =1,N_st
    if(electronic_energy(i)>h.and.electronic_energy(i).ne.0.d0)then
      ...
      e_2_pert(i) = selection_criterion*selection_criterion_factor*2.d0
        ...
        H_pert_diag(i) = h*cdabs(c_pert(i)*conjg(c_pert(i)))
        e_2_pert(i) = cdabs(conjg(c_pert(i)) * i_H_psi_array(i))
    else
      c_pert(i) = -1.d0
      e_2_pert(i) = -cdabs(i_H_psi_array(i))
      H_pert_diag(i) = h
    endif
  enddo
...
subroutine pt2_qdpt ($arguments)
  ...
  ! for the various N_st states.
  !
  ! c_pert(i) = <det_pert|H|psi(i)>/( <psi(i)|H|psi(i)> - <det_pert|H|det_pert> )
  !
  END_DOC
  
  integer                        :: i,j
  double precision               :: diag_H_mat_elem_fock, h, E, diag_H_mat_elem
  complex*16               :: i_H_psi_array(N_st), hij
  integer :: degree
...
!    delta_E = electronic_energy(1) - h
    call i_H_j(det_pert,psi_selectors(1,1,j),Nint,hij)
  ...
  do i =1,N_st
    e_2_pert(i) = cdabs(conjg(c_pert(i))*i_H_psi_array(i))
    H_pert_diag(i) = h*cdabs(conjg(c_pert(i))*c_pert(i))
  enddo
...
subroutine pt2_decontracted ($arguments)
  ...
  c_pert(1) = coef_pert
  e_2_pert(1) = cdabs(conjg(coef_pert) * i_H_psi_array(1))
  ...
  do i =1,N_st
    if (i_H_psi_array(i) /= 0.d0) then
      ...
      if (delta_e > 0.d0) then
        e_2_pert(i) = 0.5d0 * (delta_e - dsqrt(delta_e * delta_e + 4.d0 * cdabs(conjg(i_H_psi_array(i)) * i_H_psi_array(i))))
      else
        e_2_pert(i) = 0.5d0 * (delta_e + dsqrt(delta_e * delta_e + 4.d0 * cdabs(conjg(i_H_psi_array(i)) * i_H_psi_array(i))))
      endif
      if (cdabs(i_H_psi_array(i)) > 1.d-6) then
        c_pert(i) = e_2_pert(i)/conjg(i_H_psi_array(i))
      else
        c_pert(i) = 0.d0
      endif
      H_pert_diag(i) = h*cdabs(conjg(c_pert(i))*c_pert(i))
...
subroutine pt2_epstein_nesbet_SC2_projected ($arguments)
  ...
  e_2_pert(1) = cdabs(i_H_psi_array(1) * conjg(c_pert(1)))
  ...
  ! <psi|delta_H|psi>
   ...
   H_pert_diag(1) = e_2_pert(1) * cdabs(conjg(c_pert(1)) * c_pert(1))
   do i = 1, N_st
    do j = 1, idx_repeat(0)
     e_2_pert(i) += e_2_pert_fonda * cdabs(conjg(psi_selectors_coef(idx_repeat(j),i)) * psi_selectors_coef(idx_repeat(j),i))
    enddo
   enddo
  endif
...
subroutine pt2_h_core($arguments)
  use bitmasks
  implicit none
  $declarations
