...
subroutine run
  ...
  complex*16, allocatable  :: u_0(:,:), v_0(:,:)
  ...
  do j=0,nab
    ...
    if (ci_threshold_complex > norm_sort(j)) then
      cycle
    endif

    u_0(1:N_det,1:N_states) = psi_bilinear_matrix_values(1:N_det,1:N_states)
    v_0(1:N_det,1:N_states) = (0.d0,0.d0) 
    u_t(1:N_states,1:N_det) = (0.d0,0.d0)
    v_t(1:N_states,1:N_det) = (0.d0,0.d0)
    s_t(1:N_states,1:N_det) = (0.d0,0.d0)
    call cdtranspose(                                                   &
        u_0,                                                           &
        size(u_0, 1),                                                  &
        u_t,                                                           &
...
!    double precision, external :: u_dot_u, u_dot_v
    ...
    do i=1,N_states
      e_0(i) = real(u_dot_v_complex(u_0(1,i),v_0(1,i),N_det))/u_dot_u_complex(u_0(1,i),N_det)
