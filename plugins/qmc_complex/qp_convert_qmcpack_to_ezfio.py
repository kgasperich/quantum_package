...
def print_mo_coef(mo_coef_block, l_l_sym, mo_name="MO"):
    print ""
    print "BEGIN_"+mo_name
    print ""
    len_block_curent = 0
...
mo_coef = ezfio.get_mo_basis_mo_coef_real()
mo_coef_transp = zip(*mo_coef)
mo_coef_block = chunked(mo_coef_transp, 4)
print_mo_coef(mo_coef_block, l_l_sym_ordered)

mo_coef_imag = ezfio.get_mo_basis_mo_coef_imag()
mo_coef_transp_imag = zip(*mo_coef_imag)
mo_coef_block_imag = chunked(mo_coef_transp_imag, 4)
print_mo_coef(mo_coef_block_imag, l_l_sym_ordered,"MO_IMAG")
...
psi_coef = ezfio.get_determinants_psi_coef_real()
psi_coef_im = ezfio.get_determinants_psi_coef_imag()
...
psi_coef_im_small = psi_coef_im[state]
...
MultiDetAlpha = []
MultiDetBeta = []
for coef, coef_im, (det_a, det_b) in zip(psi_coef_small, psi_coef_im_small, psi_det):
...
groupMultiDet.create_dataset("Coeff_imag",(len(psi_coef_im_small),),dtype="f8",data=psi_coef_im)
